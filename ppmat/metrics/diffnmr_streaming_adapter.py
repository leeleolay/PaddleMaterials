...
class DiffNMRStreamingAdapter(StreamingMetricBase):
    ...
    def _update_eval(self, result: Dict[str, Any], batch: Any):
        assert self.model is not None, "Adapter missing model. Call adapter.bind(model=...) first."
        pred = result.get("pred_dict")  or {}
        lab  = result.get("label_dict") or {}
        node_mask = _coalesce(
        ...
        # node_mask = result.get("node_mask", None) or (batch.get("node_mask", None) if isinstance(batch, dict) else None)
        ...
        if any(v is None for v in (pred_X, pred_E, pred_y, true_X, true_E, true_y, node_mask)):
            return
        ...
        if isinstance(terms, dict):
            ...
            for k in ("nll", "X_kl", "E_kl", "X_logp", "E_logp"):
                if k in terms and terms[k] is not None:
                    self.eval_acc.add(key=_name_map(k), value=terms[k], batch_size=B, scale=self.t_scale)
        else:
...
class EvalAccumulator:
    ...
    def _to_float(x) -> float:
        if isinstance(x, paddle.Tensor):
            return float(x.numpy().item()) if x.numel() == 1 else float(paddle.sum(x).numpy().item())
        return float(x)
    ...
    def add(
        ...
        *, 
        ...
        value:
          ...
          - tensor scalar / python number → treat as mean and multiply by denom (or batch_size)
        denom:
