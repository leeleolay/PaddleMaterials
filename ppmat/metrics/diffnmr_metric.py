...
# Optimized DiffNMR metrics: CE refactor, vectorization, distributed reduction, numerical stability.
...
def _to_f32(x) -> paddle.Tensor:
    return paddle.to_tensor(x, dtype="float32") if not isinstance(x, paddle.Tensor) else x.astype("float32")
...
class SumExceptBatchMetric(paddle.metric.Metric):
    ...
    def __call__(self, values: paddle.Tensor):
        self.reset(); self.update(values); return self.accumulate()
...
class SumExceptBatchKL(paddle.metric.Metric):
    ...
    def __call__(self, p: paddle.Tensor, q: paddle.Tensor):
        self.reset(); self.update(p, q); return self.accumulate()
...
class CrossEntropyMetric(paddle.metric.Metric):
    ...
    def __call__(self, logits: paddle.Tensor, target_onehot: paddle.Tensor):
        self.reset(); self.update(logits, target_onehot); return self.accumulate()
...
class _CETracker:
    ...
    """
    def __init__(self, num_classes: int, class_names: Optional[List[str]] = None, prefix: str = ""):
        self.C = num_classes
    ...
    @staticmethod
    def _flatten_logits_targets(logits: paddle.Tensor, target_onehot: paddle.Tensor) -> Tuple[paddle.Tensor, paddle.Tensor, paddle.Tensor]:
        """
        ...
        l = paddle.reshape(logits, [-1, C])
        return l, t, mask

    def update(self, logits: paddle.Tensor, target_onehot: paddle.Tensor):
        l, t, mask = self._flatten_logits_targets(logits, target_onehot)
...
class TrainMolecularMetricsDiscrete(nn.Layer):
    ...
    def __init__(self, dataset_infos):
        ...
        self.atom_ce = _CETracker(num_classes=len(atom_names), class_names=atom_names, prefix="train/atom")
        # Bond: NoBond=0, Single=1, Double=2, Triple=3, Aromatic=4
        bond_names = ["NoBond", "Single", "Double", "Triple", "Aromatic"]
        self.bond_ce = _CETracker(num_classes=len(bond_names), class_names=bond_names, prefix="train/bond")
    ...
    def log_epoch_metrics(self) -> Dict[str, float]:
        out = {}
        out.update({k.replace("train/", "train_epoch/"): v for k, v in self.atom_ce.compute().items()})
        out.update({k.replace("train/", "train_epoch/"): v for k, v in self.bond_ce.compute().items()})
        return out
...
# RDKit deps used only at sampling
...
class _NHistogram:
    ...
    def __call__(self, molecules):
        self.update(molecules); return self.accumulate()
    def __getitem__(self, i): return self.hist[i]
...
class _NodeHistogram:
    ...
    def accumulate(self): return _l1_normalize(self.hist)
    def __call__(self, molecules): self.update(molecules); return self.accumulate()
    def __getitem__(self, i): return self.hist[i]
...
class _HistMAE(paddle.metric.Metric):
    ...
    def name(self): return f"{self.prefix}HistMAE" if self.prefix else "HistMAE"
    ...
    def __call__(self, pred_hist: paddle.Tensor):
        self.update(pred_hist); return self.accumulate()
...
class SamplingMolecularMetrics(nn.Layer):
    ...
    def __init__(self, dataset_infos: Any, train_smiles: List[str],
        ...
        self.register_buffer("target_nodes", _l1_normalize(_to_f32(dataset_infos.node_types)))
        self.register_buffer("target_edges", _l1_normalize(_to_f32(dataset_infos.edge_types)))
        self.register_buffer("target_val",   _l1_normalize(_to_f32(dataset_infos.valency_distribution)))
    ...
    def forward(self, samples: Dict[str, Any], current_epoch: int, local_rank: int,
                output_dir: str, flag_test=False, log_each_molecule=False) -> Dict[str, Any]:
        to_log: Dict[str, Any] = {}
        ...
        stability, rdkit_metrics, all_smiles = compute_molecular_metrics(pred, self.train_smiles, self.di)
        if local_rank == 0:
            to_log.update(stability)
            val, uniq, nov, conn = rdkit_metrics[0]
            to_log.update({"Validity": val, "Uniqueness": uniq, "Novelty": nov, "Connected Components": conn})
            for k, v in rdkit_metrics[2].items():
                to_log[k] = v

        # 3) Histogram MAE (streaming, cross-batch safe)
        g_n   = self.gen_n(pred);     self.mae_n(g_n)
        g_nd  = self.gen_nodes(pred); self.mae_nodes(g_nd)
        g_ed  = self.gen_edges(pred); self.mae_edges(g_ed)
        g_val = self.gen_val(pred);   self.mae_val(g_val)
        ...
        for i, atom_type in enumerate(self.atom_decoder):
            to_log[f"molecular_metrics/{atom_type}_dist"] = float((g_nd[i] - self.target_nodes[i]).item())
        for j, bond_type in enumerate(["No bond", "Single", "Double", "Triple", "Aromatic"]):
            to_log[f"molecular_metrics/bond_{bond_type}_dist"] = float((g_ed[j] - self.target_edges[j]).item())
    ...
    def _retrieval_metrics(self, samples: Dict[str, Any], output_dir: str, epoch: int, local_rank: int, *, verbose=False) -> Dict[str, float]:
        ...
        with paddle.no_grad():
            ...
            mol_flat: paddle.Tensor = self.molVec(X_in, E_in, y_in, node_mask_flat)  # [C·B, d]
            mol_embs = mol_flat.reshape([C, B, -1])                                  # [C,B,d]

        sims = F.cosine_similarity(nmr_emb.unsqueeze(0).expand([C, -1, -1]), mol_embs, axis=-1)  # [C,B]
        ...
        return {f"retrieval_top{k}": h / len(true_list) for k, h in zip(ks, hits) if C >= k}
...
class DiffNMRMetric:
    ...
    def _ready(self) -> bool:
        ...
        if self.mode == "sample":
            return (self._dataset_infos is not None) and (self._train_smiles is not None)
        return False
    ...
    def _build_impl(self):
        if self.mode == "train":
            ...
            self.impl = SamplingMolecularMetrics(self._dataset_infos, self._train_smiles, self._clip, self._num_candidate)
        else:
    ...
    def __call__(self, pred, label=None, **kwargs):
        ...
        if self.mode == "train":
            return self.impl(
                pred["masked_pred_X"], pred["masked_pred_E"],
                label["true_X"], label["true_E"],
