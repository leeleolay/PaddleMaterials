...
def step(
    ...
    out_one_hot = diffgraphformer_utils.PlaceHolder(X=X_s, E=E_s, y=paddle.zeros([y_t.shape[0], 0]))
    out_discrete = diffgraphformer_utils.PlaceHolder(X=X_s, E=E_s, y=paddle.zeros([y_t.shape[0], 0]))
...
def apply_noise(model, X, E, y, node_mask, flag_use_formula = None):
    ...
    z_t = diffgraphformer_utils.PlaceHolder(X=X_t, E=E_t, y=y).type_as(X_t).mask(node_mask)
...
def compute_val_loss(
    ...
    Args:
        ...
        return_terms:      if True, return per-sample vector terms dict instead of a scalar.
    ...
    # loss_term_0 = model.val_X_logp(loss_term_0_x) + model.val_E_logp(loss_term_0_e) TODO
...
def compute_Lt(
    ...
    if return_terms is False:
        ...
        (loss_all_t, x_kl, e_kl) : 3 Tensors[B]
           x_kl, e_kl are per-sample KL(P_true || P_pred) contributions (already scaled by model.T).
    """
...
def reconstruction_logp(model, t, X, E, node_mask, condition_Spectrum):
    ...
    if model.__class__ == DiffNMR:
        ...
        # prepare the extra feature for encoder input without noisy
        z_t = diffgraphformer_utils.PlaceHolder(X=X0, E=E0, y=y0).type_as(X).mask(node_mask)
        extra_data_pure = compute_extra_data(
            model,
