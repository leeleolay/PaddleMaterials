...
class MSDnmrDataset(Dataset):
    ...
    **Data Format**
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if build_graph_cfg is None:
            build_graph_cfg = {
                "atom_vocab": {"H": 0, "C": 1, "N": 2, "O": 3, "F": 4, "P": 5, "S": 6, "Cl": 7, "Br": 8, "I": 9},
        ...
        self.vocabs = self._build_vocab(
            vocab_peakwidth_path, vocab_split_path
        )
            ...
            if build_spectrum_cfg is not None and not overwrite:
                try:
                    ...
                    if is_equal(build_spectrum_cfg_cache, build_spectrum_cfg):
                        ...
                        logger.warning(
                            "build_spectrum_cfg is different from build_spectrum_cfg_cache"
        ...
        if overwrite or not self.cache_exists:
            ...
            if dist.get_rank() == 0:
                ...
                if build_spectrum_cfg is not None:
                    converter = build_spectrum_converter(build_spectrum_cfg, vocabs=self.vocabs, strict=True)
                    ...
                    logger.info(f"Save {self.num_samples} spectrums to {spectrum_cache_path}")
    ...
    def read_data(self, csv_path: Union[str, List[str]]) -> Tuple[List[str], List[dict], List[int], int]:
        ...
        if isinstance(csv_path, (list, tuple)):
            ...
            file_list = [osp.join(csv_path, f) for f in os.listdir(csv_path) if f.endswith(".csv")]
            file_list.sort()
        else:
            file_list = [csv_path]
        ...
        df["atom_count"] = pd.to_numeric(df["atom_count"], errors="coerce").astype("Int64")
    ...
    def filter_by_atom_count_raw(
        ...
        Returns:
            reserve_idx (List[int]): kept indices (when inplace=True, also mutates datasets).
        ...
        try:
            ...
            logger.warning(
                ...
                f"min={min_atoms}, max={max_atoms}, allowed={set(allowed) if allowed is not None else None}"
            )
        except Exception:
            pass
    ...
    def get_molecule_array(self, molecule):
        ...
        atom_vocab   = getattr(self, "atom_vocab", {"H":0,"C":1,"N":2,"O":3,"F":4,"P":5,"S":6,"Cl":7,"Br":8,"I":9})
        bond_vocab   = getattr(self, "bond_vocab", (BT.SINGLE, BT.DOUBLE, BT.TRIPLE, BT.AROMATIC))
        ...
        if N == 0:
            ...
            return {
                ...
                "node_feat": ConcatData(np.zeros((0, len(atom_vocab)), dtype=np.float32)),
                "edge_feat": ConcatData(np.zeros((0, len(bond_vocab)+1), dtype=np.float32)),
            }
        ...
        def push(u, v, et):
            rows.append(u); cols.append(v); etypes.append(et)
        ...
        if add_self:
            for i in range(N):
                rows.append(i); cols.append(i); etypes.append(0)
...
class MSDnmrinfos:
    def __init__(self, dataloaders, cfg, recompute_statistics=False):
        ...
        if self.dataflag == "n<15":
            ...
            logger.message(f"invalid dataflag: %s", self.dataflag)
        ...
        self.train_smiles = get_train_smiles(cfg, dataloaders.train_dataloader, self, evaluate_dataset=False)
...
def get_train_smiles(cfg, dataloader, dataset_infos, evaluate_dataset=False):
    ...
    if not osp.exists(cfg["datadir"]):
        logger.message("The dataset directory is not found. Will save it to default path now.")
        root_path = download.get_datasets_path_from_url(MSDnmrDataset.url, MSDnmrDataset.md5)
        ...
        if cfg["data_flag"] == "n<15":
            ...
            raise ValueError(f"Unknown data_flag: {cfg['data_flag']}. Expected one of {'n<15', 'n<20', 'n<30', 'n<35'}.")
...
def compute_MSDnmr_smiles(atom_decoder, dataloader, remove_h):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if i % 1000 == 0:
            logger.message(f"Converting MSDnmr dataset to SMILES {float(i)/len_train:.2%}")
