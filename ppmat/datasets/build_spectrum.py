...
def build_spectrum_converter(
    ...
    *,
    ...
    if not class_name:
        raise ValueError("Spectrum converter class name is not specified in the configuration.")
    ...
    if strict and hasattr(cls, "REQUIRED_VOCABS"):
        ...
        miss = req - got
        if miss:
            raise ValueError(f"{class_name} is missing required vocabularies: {sorted(miss)}")
        
        unk = str(init_params.get("unk_token", "<unk>"))

        for name, vb in (init_params.get("vocabs") or {}).items():
            if unk not in vb:
                raise ValueError(f"Vocabulary '{name}' must include the unknown token '{unk}'")
...
class BuildSpectrumNMR:
    ...
    Output (dict of NumPy arrays and counts):
        {
          "H_nmr":      float32 [seq_len_H1, 4 + j_len],  # [δ, peakwidth_id, split_id, integral, J*]
          "num_H_peak": int,
          "C_nmr":      float32 [seq_len_C13],            # δ (ppm), padded/truncated
          "num_C_peak": int,
        }
    ...
    Notes:
        ...
        - Integral like "3H" is parsed as 3; you can add a constant offset if that matches your training setup.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if self.unk_token not in self.vocab_peakwidth or self.unk_token not in self.vocab_split:
            raise ValueError(f"Both vocabs must contain the unknown token '{self.unk_token}'.")
    ...
    def build_one(
        ...
        # Allocate [seq_len_H1, 4 + j_len]: [δ, peakwidth_id, split_id, integral, J1..Jj_len]
        ...
        for i in range(limit_h):
            peak = Hnmr[i]
            # Expected: [chem_shift (float), peakwidth_token (str), split_token (str), "nH", [J...]]
            chem_shift = float(peak[0])
            peakwidth_tok = str(peak[1])
            split_tok = str(peak[2])
            integral_str = peak[3]
            j_list = peak[4] if len(peak) > 4 and isinstance(peak[4], (list, tuple)) else []

            peakwidth_id = vocab_peakwidth.get(peakwidth_tok, vocab_peakwidth[unk_token])
    ...
    def __call__(self, nmr_list: Union[Sequence[Dict[str, Any]], Dict[str, Any]]) -> Union[List[Dict[str, Any]], Dict[str, Any]]:
