...
from typing import Optional, List, Dict, Tuple, Optional, Sequence,  Union
...
class MolecularGraphConverter:
    ...
    def __init__(
        ...
        if atom_vocab is None:
            if remove_h is False:
                ...
                atom_vocab = {"C": 0, "N":1, "O": 2, "F": 3, "P": 4, "S": 5, "Cl": 6, "Br": 7, "I": 8}
    ...
    def build_one(
        ...
        for atom in mol.GetAtoms():
            ...
            if sym not in atom_vocab:
                return None  # Unknown Elements: Can be replaced with an extended vocabulary or placeholder <unk>
        ...
        def push(u, v, et): rows.append(u); cols.append(v); etypes.append(et)
        ...
        # 3）Hydrogen removal on the graph (masking + relabeling). The Mol remains unchanged.
        if remove_h:
            ...
            if h_id is not None:
                ...
                keep_cols = np.array([i for i in range(len(atom_vocab)) if i != h_id], dtype=np.int64)
        ...
        if add_self_loops and N_new > 0:
            self_e2 = np.stack([np.arange(N_new), np.arange(N_new)], axis=1).astype(np.int64)
            self_ea = np.eye(len(bond_vocab) + 1, dtype=np.float32)[np.zeros((N_new,), dtype=np.int64)]
    ...
    def __call__(self, mols: Union[Sequence[Chem.Mol], Chem.Mol]) -> Union[List[pgl.Graph], pgl.Graph, None]:
...
def subgraph(
    ...
    *,
    ...
    if isinstance(subset, (list, tuple, np.ndarray)) and (not np.asarray(subset).dtype == bool):
    ...
    if keep_idx.size == 0:
        ...
        new_edge_attr  = (np.empty((0, edge_attr.shape[1]), dtype=edge_attr.dtype)
