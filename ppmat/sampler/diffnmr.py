...
class DiffNMRSampler:
    ...
    def __init__(
        ...
        if model_name is None:
            ...
            checkpoint_path = resolve_model_package_dir(model_name, extracted_path)
            config_path = get_model_config_path(model_name, checkpoint_path)
            package_config_dir = checkpoint_path
            config = OmegaConf.load(config_path)
            if config_overrides:
                cli_config = OmegaConf.from_dotlist(config_overrides)
                config = OmegaConf.merge(config, cli_config)
                ...
                for key, value in obj.items():
                    if key in {
                        ...
                        if key in {
                            "vocab_peakwidth_path",
                            "vocab_split_path",
                        } and not osp.isfile(resolved_path):
                            raise FileNotFoundError(
                                f"DiffNMR package is missing required {key}: "
                                f"{resolved_path}"
                            )
