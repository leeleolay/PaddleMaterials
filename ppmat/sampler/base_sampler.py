...
class MolecularSampler:
    ...
    def __init__(
        ...
        if model_name is None:
            ...
            dataset_infos = build_dataset_infos(dataloaders=dataloaders, cfg=config, recompute_statistics=False)
            ...
            self.pretrained_model_path = checkpoint_path if checkpoint_path is not None else config.get("pretrained_model_path", None)
            self.pretrained_weight_name = weights_name if weights_name is not None else config.get("pretrained_weight_name", None)
        ...
        self.metric_dict_sample = sample_config.get(
            "out_dict", None
        )
        ...
        self.flag_retrival_initilization = sample_config.get("flag_retrival_initilization", False)
    ...
    def sample_epoch(
        ...
        for iter_id, batch_data in enumerate(dataloader):
            ...
            # basic batch tensors
            batch_atomCount = paddle.to_tensor(batch_property["atom_count"])           # [B] number of atoms
            ...
            for c_idx in range(num_candidates):
                kwargs = dict(
                    model          = self.model._layers if isinstance(self.model, paddle.DataParallel) else self.model,
                    batch_id       = iter_id,
        ...
        self.streaming.update_step(
            result={"samples": samples, "epoch_id": epoch_id, "local_rank": self.rank, "output_dir": self.output_dir},
            batch=None, stage="sample"
        )
    ...
    def sample_batch(
        ...
        if retrival_initilization and batch_condition is not None:
            ...
            similarities = self._batched_cosine_similarity(output, molecular_vectors, 128)
            top_k = 1
    ...
    def _init_retrieval_bank(self, cfg):
        ...
        try:
            if ext == ".csv":
                ...
                # to paddle tensor
                embs = paddle.to_tensor(np.stack(data["molecularRep"].values), dtype="float32")
