...
from ppmat.datasets.build_field import normalize_coordinate_unit
...
def use_reference_atom_coordinates(
    ...
    expected_atom_numbers = np.asarray(
        [
            ...
            for atom_type in _to_numpy(_graph_node_feature(graph, "x")).reshape(-1)
        ],
        dtype=np.int64,
    )
    if (
    ...
    return graph_converter.from_arrays(
        ...
        node_features={
            "x": _to_numpy(_graph_node_feature(graph, "x")).reshape(-1),
        },
    )
...
class FieldPredictor(BasePredictor):
    ...
    def __init__(
        ...
        self.graph_converter = build_graph_converter(
        ...
        if not hasattr(self.graph_converter, "from_arrays"):
            raise TypeError(
                "Predict.build_graph_cfg must build an array-compatible graph "
                "converter."
            )
        if (
            getattr(self.graph_converter, "coordinate_unit", None)
    ...
    def from_data(
        ...
        atom_types = np.asarray(_graph_node_feature(graph, "x")).reshape(-1)
        atom_coord = np.asarray(_graph_node_feature(graph, "pos"))
    ...
    def from_dataset(
        ...
        if split_file_path is not None:
            dataset_params["split_file"] = split_file_path
    ...
    def _save_cubes(
        ...
        atom_type = _to_numpy(_graph_node_feature(graph, "x")).reshape(-1)
        atom_coord = _to_numpy(_graph_node_feature(graph, "pos"))
