...
def _atomic_number_from_symbol(symbol):
    if symbol.startswith("X") and symbol[1:].isdigit():
        return int(symbol[1:])
    return ASE_ATOMIC_NUMBERS[symbol]
    ...
    return (
        ...
        np.asarray(grid.cartesian_coordinates(), dtype=np.float32),
        {
            ...
            "atom_numbers": np.asarray(
                [
                    _atomic_number_from_symbol(symbol)
...
def prepare_cube_info(
    ...
    """Validate an explicit grid and return its CUBE geometry metadata."""
    ...
    cell = _to_numpy(info["cell"]).astype(np.float32, copy=False)
    origin = _to_numpy(info["origin"]).astype(np.float32, copy=False)
    ...
    actual_points = _to_numpy(grid_coord).reshape(-1, 3)
