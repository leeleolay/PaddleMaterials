...
def test_model_package_helpers_resolve_standard_zip_layout(tmp_path):
    ...
    resolved_path = resolve_model_config_path("infgcn_qm9", str(cache_dir))
    assert Path(resolved_path) == config_path


def test_model_package_helpers_accept_downloaded_package_dir(tmp_path):
    from ppmat.utils.resource import resolve_model_config_path
    ...
    resolved_path = resolve_model_config_path("infgcn_qm9", str(package_dir))
    assert Path(resolved_path) == config_path


def test_model_package_helpers_have_focused_module():
    resource_source = (ROOT / "ppmat/utils/resource.py").read_text()
    generic_io_source = (ROOT / "ppmat/utils/io.py").read_text()

    assert "def resolve_model_config_path" in resource_source
    assert "def resolve_model_package_dir" not in resource_source
    assert "def get_model_config_path" not in resource_source
...
def test_build_model_from_name_requires_standard_package():
    ...
    assert "resolve_model_config_path(model_name, extracted_path)" in source
...
def test_field_predictor_reuses_base_and_keeps_helpers_outside_predictor():
    ...
    assert "def normalize_coordinate_unit(" in build_field_source
...
def test_all_infgcn_configs_are_parseable_and_complete():
    ...
    for config_path in config_paths:
        ...
        dataset_format = expected_dataset_formats.get(config_path.stem, "array")
        ...
        for split in ["train", "val", "test"]:
            ...
            location_key = (
                "root" if dataset["__class_name__"] == "MD17DensityDataset" else "path"
            )
            assert location_key in dataset["__init_params__"], config_path.name
            ...
            assert (
                unresolved_init_params["build_field_cfg"] == "${Global.build_field_cfg}"
            )
            assert (
            ...
            if config_path.stem == "infgcn_omol25_MC_5k_trimmed" and split in {
                ...
                assert (
                    split_cfg["loader"]["collate_params"]["resample_each_call"] is False
                )
...
def test_electronic_structure_train_default_config_exists(monkeypatch):
    from electronic_structure.train import parse_args

    monkeypatch.setattr(sys, "argv", ["electronic_structure/train.py"])
