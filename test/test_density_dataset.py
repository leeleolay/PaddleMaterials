...
@pytest.mark.parametrize(
    "parameter",
    ["auto_download", "url", "md5", "max_cache_samples", "rotate", "pbc"],
)
...
def _md17_density_dataset(root, split, **kwargs):
    params = dict(
        ...
        vocab=_atom_vocab(),
        build_graph_cfg=_graph_converter_cfg("bohr"),
        build_field_cfg=_field_converter_cfg("array", "unknown", "bohr"),
    )
    params.update(kwargs)
...
def test_md17_density_download_uses_shared_download_utils(tmp_path, monkeypatch):
    ...
    assert train.root == str(canonical_root)
    assert validation.root == str(canonical_root)
    assert raw_downloads == 1


def test_md17_density_dataset_requires_field_config(tmp_path):
    root = tmp_path / "small"
    _write_md17_density_source(root)

    with pytest.raises((TypeError, ValueError), match="build_field_cfg"):
        _md17_density_dataset(root, "train", build_field_cfg=None)
...
def test_md17_converter_configs_are_required_keyword_only():
    ...
    for parameter_name in ("vocab", "build_field_cfg", "build_graph_cfg"):
...
def test_md17_cache_build_converts_bounded_batches(tmp_path, monkeypatch):
    ...
    original_convert_fft = MD17DensityDataset._convert_fft
    batch_sizes = []
    log_messages = []

    def tracking_convert_fft(self, fft_coeff):
        batch_sizes.append(fft_coeff.shape[0])
        return original_convert_fft(self, fft_coeff)

    monkeypatch.setattr(density_dataset_module, "_MD17_CACHE_BATCH_SIZE", 3)
    monkeypatch.setattr(MD17DensityDataset, "_convert_fft", tracking_convert_fft)
    monkeypatch.setattr(
        density_dataset_module.logger,
        "message",
...
def test_md17_density_dataset_fft_conversion_stays_numpy():
    dataset = MD17DensityDataset.__new__(MD17DensityDataset)
    dataset.split = "train"
    dataset.n_grid = 2

    density = dataset._convert_fft(np.arange(8, dtype=np.float32).reshape(1, -1))
...
def test_md17_density_dataset_rebuilds_stale_samples(tmp_path):
    ...
    sample_dir = root / "ethane" / "ethane_test_cache" / "samples"
...
def test_md17_density_dataset_rebuilds_truncated_sample(tmp_path):
    ...
    sample_path = root / "ethane" / "ethane_test_cache" / "samples" / "0000000000.pkl"
