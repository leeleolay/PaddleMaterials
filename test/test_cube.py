...
def test_cube_builder_preserves_atomic_numbers_beyond_cvve_symbol_table(tmp_path):
    ...
    np.testing.assert_array_equal(info["atom_numbers"], values["atom_numbers"])
...
def test_cube_writer_rejects_cell_shorthand(tmp_path):
    values = _cube_values()
    values["info"]["cell"] = np.asarray([2.0, 3.0, 4.0])
